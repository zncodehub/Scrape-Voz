...
├── thread_viewer.html          (Master HTML structure template dynamically cloned into thread subfolders)
├── viewer.css                  (Global visual stylesheet file cloned into thread subfolders)
├── viewer.js                   (Global application script logic dynamically cloned into thread subfolders)
...
└── threads/                    (Scraped library - ignored from Git version control)
    └── [thread_folder]/        (Named using convention: <thread_id>_<20_char_slug>…)
        ...
        ├── viewer.css          (Copy of viewer.css cloned post-scraping)
        ├── viewer.js           (Copy of viewer.js cloned post-scraping)
